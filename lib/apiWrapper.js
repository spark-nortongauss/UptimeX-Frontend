...
export async function apiRequest(resource, options = {}) {
  ...
  try {
    ...
    // Never mutate error.message — it is read-only on TypeError in some envs.
    // Wrap in a new Error instead.
    throw new Error(`[apiRequest] ${error?.message ?? String(error)}`);
  }
...
export function unwrapResponse(summary, fallbackMessage = 'Request failed') {
  ...
  if (!summary.ok) {
    const errorMessage = summary.data?.message || fallbackMessage;
    // Build a plain object instead of mutating Error properties.
