...
export const useMonitoringDataStore = create((set, get) => ({
  ...
  getTemperatureItems: () => {
    ...
    if (process.env.NODE_ENV === 'development' && temperatureItems.length > 0) {
      console.log(`[TemperatureChart] Found ${temperatureItems.length} temperature items:`, 
        temperatureItems.map(item => ({ name: item.name, key: item.key, units: item.units }))
      );
    }
