...
class WorkspaceService {
  ...
  async bulkDeleteWorkspaces(token, workspaceIds) {
    const response = await fetch(`${API_BASE_URL}/workspaces/bulk-delete`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || 'Failed to delete workspaces');
    }
