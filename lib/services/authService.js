...
export const authService = {
  ...
  async isAdmin(token) {
    try {
      ...
      const role = verify?.user?.dbUser?.role
      return role === 'ADMIN'
    } catch (error) {
      return false
    }
  ...
  async requestPasswordReset(email) {
    const response = await apiRequest(`${API_BASE_URL}/auth/forgot-password/send-otp`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email }),
    });
    return unwrapResponse(response, 'Failed to send verification code');
  },

  async verifyPasswordResetOtp(email, otp) {
    const response = await apiRequest(`${API_BASE_URL}/auth/forgot-password/verify-otp`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email, otp }),
    });
    return unwrapResponse(response, 'Failed to verify code');
  },

  async resetPasswordWithOtp({ email, resetToken, newPassword }) {
    const response = await apiRequest(`${API_BASE_URL}/auth/forgot-password/reset`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
