...
class ZabbixService {
  ...
  async makeRequest(endpoint, options = {}) {
    try {
      ...
      return unwrapResponse(response, `Monitoring API request failed for ${endpoint}`);
    } catch (error) {
      console.error(`Monitoring API request failed for ${endpoint}:`, error);
      throw error;
    }
  }
  ...
  async logout() {
    try {
      ...
      console.error('Monitoring logout failed:', error);
      throw error;
    }
  }
  ...
  async checkAuthentication() {
    try {
      ...
      console.error('Monitoring authentication check failed:', error);
  ...
  async getApiInfo() {
    try {
      ...
      console.error('Failed to get Monitoring API info:', error);
      throw error;
    }
  }
  ...
  async testConnection() {
    try {
      ...
      console.error('Monitoring connection test failed:', error);
  ...
  async getAllData() {
    try {
      ...
      console.error('Failed to get all monitoring data:', error);
      throw error;
    }
  }
  ...
  // RF Metrics methods
  async getRfMetrics(hostId, options = {}) {
    try {
      const queryString = new URLSearchParams();
      ...
      // Use full API base URL for observone endpoints (not zabbix base)
      const headers = await this.getAuthHeaders();
      const endpoint = `${API_BASE_URL}/observone/rf-metrics/${hostId}${queryString.toString() ? `?${queryString.toString()}` : ''}`;
      ...
      return unwrapResponse(response, `RF metrics API request failed for host ${hostId}`);
    } catch (error) {
      console.error(`Failed to get RF metrics for host ${hostId}:`, error);
      throw error;
    }
  }
  ...
  async getServiceAvailability(params = {}) {
    try {
      ...
      if (params.hostids) {
        const ids = Array.isArray(params.hostids) ? params.hostids : [params.hostids];
        ids.forEach((id) => queryString.append('hostids', id));
      }
      if (params.severity) {
        const severities = Array.isArray(params.severity) ? params.severity : [params.severity];
        severities.forEach((sev) => queryString.append('severity', sev));
      }
      if (params.limit) {
        queryString.append('limit', params.limit);
      }
      if (params.period_days) {
        queryString.append('period_days', params.period_days);
      }
      if (params.period_from) {
        queryString.append('period_from', params.period_from);
      }
      if (params.period_to) {
        queryString.append('period_to', params.period_to);
      }
      if (params.include_disabled !== undefined) {
        queryString.append('include_disabled', params.include_disabled ? 'true' : 'false');
      }

      const endpoint = `${API_BASE_URL}/api/services${queryString.toString() ? `?${queryString.toString()}` : ''}`;
      const response = await apiRequest(endpoint, {
        method: 'GET',
        headers,
      });

      return unwrapResponse(response, 'Failed to load services availability');
    } catch (error) {
      console.error('Failed to load services availability:', error);
      throw error;
    }
  }

  async getOpticalStatus(hostId, options = {}) {
    try {
      const queryString = new URLSearchParams();
      ...
      if (options.limitPerItem) {
        queryString.append('limit', options.limitPerItem);
      }

      const headers = await this.getAuthHeaders();
      const endpoint = `${API_BASE_URL}/api/hosts/${hostId}/optical-status${queryString.toString() ? `?${queryString.toString()}` : ''}`;
      ...
      return unwrapResponse(response, `Optical status API request failed for host ${hostId}`);
    } catch (error) {
      console.error(`Failed to get optical status for host ${hostId}:`, error);
      throw error;
    }
  }
}
