...
export async function generateReportPDF(systemId, chartRefs = {}, onProgress) {
  try {
    ...
    const buildOpticalLabelFromKey = (key) => {
      if (!key) return 'Optical Level'
      if (key.includes('__')) {
        const [ruPart, typePart] = key.split('__')
        const prettyRu = ruPart
          .split('-')
          .filter(Boolean)
          .map((segment) => segment.replace(/\b\w/g, (char) => char.toUpperCase()))
          .join(' ')
        const typeLabel = typePart ? `${typePart.toUpperCase()} Level` : 'Level'
        return `${prettyRu} ${typeLabel}`.trim()
      }
      const pdMatch = key.match(/pdSector(\d+)/i)
      if (pdMatch) {
        return `Sector ${pdMatch[1]} PD Level`
      }
      const ldMatch = key.match(/ldSector(\d+)/i)
      if (ldMatch) {
        return `Sector ${ldMatch[1]} LD Level`
      }
      return key
    }
    ...
    if (chartInstances.optical) {
      const opticalEntries = Object.entries(chartInstances.optical).filter(
        ([, ref]) => ref && (ref.current !== undefined || ref.ref || ref.label !== undefined)
      )

      if (opticalEntries.length > 0) {
        checkNewPage(80)
        doc.setFontSize(16)
        doc.setFont(undefined, 'bold')
        ...
        const sortedEntries = opticalEntries.sort((a, b) => {
          const aLabel = (a[1]?.label || buildOpticalLabelFromKey(a[0]) || '').toLowerCase()
          const bLabel = (b[1]?.label || buildOpticalLabelFromKey(b[0]) || '').toLowerCase()
          return aLabel.localeCompare(bLabel, undefined, { numeric: true })
        })

        for (const [key, refInfo] of sortedEntries) {
          const label = refInfo?.label || buildOpticalLabelFromKey(key)
          const refObject = refInfo?.ref ? refInfo.ref : refInfo
          const instance = await waitForChartReady(refObject)
          if (instance) {
            try {
              const chartImage = await captureApexChart(instance)
              await addImage(chartImage, contentWidth, 70, label)
            } catch (error) {
              console.error(`Error capturing optical chart (${label}):`, error)
              doc.setFontSize(10)
